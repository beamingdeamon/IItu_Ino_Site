...
def firstGetMessage(message):
    response = requests.get(f'{backend_url}/api/getpartners/')
    data = response.text
    if len(data) > 4096:
...
def deletePartner(message):
    response = requests.delete(f'{backend_url}/api/deletepartner/' + message.text + '/')
...
def updateData(message, partner_id):
    response = requests.put(f'{backend_url}/api/updatepartner/' + partner_id + '/', json={"text": message.text})
    if (response.status_code == 200):
